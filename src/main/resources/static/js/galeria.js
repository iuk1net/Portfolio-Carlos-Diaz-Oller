...
(function() {
    ...
    class GaleriaManager {
            ...
            if (inputFile) {
                inputFile.addEventListener('change', (e) => {
                    ...
                    files.forEach((file, index) => {
                        this.subirImagen(file, index === 0); // Primera imagen es principal
                    });
        ...
        async subirImagen(file, esPrincipal = false) {
            // Validar tamaño (5MB)
            if (file.size > 5 * 1024 * 1024) {
                this.mostrarNotificacion('La imagen excede 5 MB', 'error');
                return;
            }

            // Validar tipo
            if (!file.type.startsWith('image/')) {
                this.mostrarNotificacion('Solo se permiten imágenes', 'error');
                return;
            }
            ...
            try {
                ...
                this.mostrarNotificacion(error.message || 'Error al subir imagen', 'error');
                preview.remove();
            }
        }
